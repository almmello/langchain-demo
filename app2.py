...
template = """
        Given the {category}, please write a detailed travel journal for astronauts on the planet for ten days. 
        Talk about their experiences and about the spaceships every day. Each day must have its own page with an hourly mission description. 
        If there is missing information, you should create it as fiction.
        ---
        Category: {category}
        Prompt: {prompt}
        ---
...
class MyPromptTemplate(PromptTemplate):
    input_variables = ["prompt", "category"]
    template = template

prompt_template = MyPromptTemplate()
prompt = prompt_template.format(prompt="Write a ten-day detailed travel journal", category="planet")
